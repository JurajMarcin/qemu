...
int inet_parse(InetSocketAddress *addr, const char *str, Error **errp)
{
    ...
    if (!addr_str) {
        error_setg(errp, "address is required");
        return -1;
    }
    if (addr_str[0] == ':') {
        /* no host given */
        host[0] = '\0';
        if (sscanf(addr_str, ":%32[^,]%n", port, &pos) != 1) {
            error_setg(errp, "error parsing port in address '%s'", addr_str);
            return -1;
        }
    } else if (str[0] == '[') {
        /* IPv6 addr */
        if (sscanf(addr_str, "[%64[^]]]:%32[^,]%n", host, port, &pos) != 2) {
