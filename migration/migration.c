...
bool migration_is_running(void)
{
    ...
    case MIGRATION_STATUS_POSTCOPY_RESUMABLE:
...
static bool migration_is_active(void)
{
    ...
    return (s->state == MIGRATION_STATUS_ACTIVE ||
            s->state == MIGRATION_STATUS_POSTCOPY_RESUMABLE ||
    ...
    while (migration_is_running()) {
        ...
        case MIG_RP_MSG_PONG:
            ...
            if (tmp32 == QEMU_VM_PING_PACKAGED_LOADED) {
                ...
                migrate_set_state(&ms->state, MIGRATION_STATUS_POSTCOPY_RESUMABLE,
                                  MIGRATION_STATUS_POSTCOPY_ACTIVE);
            }
            break;
...
static int postcopy_start(MigrationState *ms, Error **errp)
{
    ...
    migrate_set_state(&ms->state, MIGRATION_STATUS_DEVICE,
                      MIGRATION_STATUS_POSTCOPY_RESUMABLE);
...
static void migration_completion(MigrationState *s)
{
    ...
    } else if (s->state == MIGRATION_STATUS_POSTCOPY_RESUMABLE ||
               s->state == MIGRATION_STATUS_POSTCOPY_ACTIVE) {
...
static MigIterateState migration_iteration_run(MigrationState *s)
{
    ...
    Error *local_err = NULL;
    bool in_postcopy = (s->state == MIGRATION_STATUS_POSTCOPY_RESUMABLE ||
                        s->state == MIGRATION_STATUS_POSTCOPY_ACTIVE);
