    ...
    QTAILQ_FOREACH(client, &exp->clients, next) {
        ...
        if (client->recv_coroutine) {
            if (client->read_yielding) {
                qemu_aio_coroutine_enter(exp->ctx,
                                         client->recv_coroutine);
            } else {
                AIO_WAIT_WHILE(exp->ctx, client->recv_coroutine != NULL);
            }
        }

        if (client->send_coroutine) {
            AIO_WAIT_WHILE(exp->ctx, client->send_coroutine != NULL);
        }
    }
}
...
static void blk_aio_detach(void *opaque)
{
    ...
    aio_wait_bh_oneshot(exp->ctx, nbd_aio_detach_bh, exp);
